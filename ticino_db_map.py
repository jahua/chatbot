...
def load_shapefile_data():
    ...
    try:
        # Get the current directory and construct path to shapes directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        shapes_dir = os.path.join(current_dir, 'app', 'static', 'geojson', 'shapes')
        shapefile_path = os.path.join(shapes_dir, 'g1b23.shp')
        ...
        if os.path.exists(shapefile_path):
            try:
                ...
                # Assuming 'KTNR' column exists and Ticino's number is 21
                if 'KTNR' in gdf.columns:
                    original_count = len(gdf)
                    gdf = gdf[gdf['KTNR'] == 21]
                    filtered_count = len(gdf)
                    st.sidebar.info(f"Filtered shapefile for Ticino. Kept {filtered_count}/{original_count} regions.")
                    if filtered_count == 0:
        ...
        st.error("An unexpected error occurred while trying to load shapefile data.")
        return create_fallback_data()
