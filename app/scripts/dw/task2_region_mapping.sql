...
BEGIN
    ...
    INSERT INTO dw.dim_region (
        ...
        ('Uri', 'canton', TRUE),
        ('Schwyz', 'canton', TRUE),
        ('Obwalden', 'canton', TRUE),
        ('Nidwalden', 'canton', TRUE)
...
BEGIN
    ...
    ELSE
        ...
        SELECT region_name INTO v_region_name FROM dw.dim_region WHERE region_id = v_region_id;
        RAISE NOTICE 'Test 1: "Zürich" maps to region_id % (canonical name: %)', v_region_id, v_region_name;
    END IF;
    ...
    ELSE
        -- Get the canonical name
        SELECT region_name INTO v_region_name FROM dw.dim_region WHERE region_id = v_region_id;
        RAISE NOTICE 'Test 2: "Zurich" (mastercard) maps to region_id % (canonical name: %)', v_region_id, v_region_name;
    END IF;
END;
$$;
